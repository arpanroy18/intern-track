...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (secret !== HEALTH_SECRET) {
    console.log(`[PING] Unauthorized access attempt at ${new Date().toISOString()}`);
  ...
  if (error) {
    console.log(`[PING] Supabase query failed at ${new Date().toISOString()}: ${error.message}`);
    return res.status(500).json({ error: 'Supabase query failed', details: error.message });
  }
  
  console.log(`[PING] Health check successful at ${new Date().toISOString()}`);
